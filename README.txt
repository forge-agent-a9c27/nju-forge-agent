...
6. 稳定终止：API 遇到限流、超时和服务端错误时有限退避重试；参数错误和执行异常会转换为模型可理解的结果。连续三次相同调用触发熔断，达到步数上限后撤销工具权限并生成事实性总结。
...
五、测试与文档
执行 python -m unittest discover -s tests -v 可运行 26 项离线测试，覆盖路径逃逸、敏感文件、并发冲突、回滚、命令分级、凭据隔离、上下文压缩、循环熔断、API 解析以及 CLI 到本地 HTTP 模拟模型再到文件写入和最终回答的完整链路。执行 python -m compileall -q agent.py forge_agent 可做语法检查。
